...
const asteaRequest = async (url, body, options = {}) => {
  ...
  if (resp.data.ExceptionDetail) return { error: new AsteaError(resp.data) };
...
class Astea {
  ...
  static async getMaterial(id, session) {
    const { sessionID: sessionId } = session;
    let resp = await asteaRequest(
        URLExecuteMacro,
        createMacro()
    ...
    const { error, data } = await asteaRequest(
      ...
      formatCommandBody(
        ...
        serviceOrder.isInHistory
          ? "customer_authorization_history"
  ...
  static async createInteraction(id, session, message) {
    const { HostName, StateID } = (await this.getServiceOrder(id, session))
      .serviceOrder;
    const { error, data } = await asteaRequest(URLInteractWithServer);
  }
  ...
  static async locatorSearch(session, criteria, page = 1) {
    ...
    const { error, data } = await asteaRequest(URLSearch, body, {
      headers: {
        SOAPAction:
...
async function parseSearchResults(data) {
  ...
  const resultsEncodedXML =
    data["s:Envelope"]["s:Body"][0]["RetrieveXMLExtResponse"][0][
      "RetrieveXMLExtResult"
    ][0];
  //TODO Make these nasties a little cleaner.

  const resultsXML = decodeFromAsteaGibberish(resultsEncodedXML);
  const xmlResults = await parseXMLToJSON(resultsXML).catch((err) => {
  ...
  const resultsPromises = xmlResults.root.row.map(
    async (svRawData) => await Order.parse(svRawData, false)
  );
  return {
    meta,
